...
class _HomePageState extends State<HomePage> {
  ...
  /*Future<void> loadInitialData() async {
    ...
    testId = await getTestId(context);

    DocumentReference testRef = FirebaseFirestore.instance.collection('Tests').doc(testId);
    DocumentSnapshot docSnapshot = await testRef.get();

    if (docSnapshot.exists) {
      Uint8List? imageBytes = docSnapshot.data() != null?('imageCube') as Uint8List: null;

      if (imageBytes != null) {
        imageCube = Image.memory(imageBytes);
      }else{
        print("imagem null");
      }

      setState(() {}); // Atualiza a interface do usuário após carregar os dados
    }else{
      print("deu ruim");
    }
  }*/
